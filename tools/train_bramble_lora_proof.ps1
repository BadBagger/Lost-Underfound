...
$out = "D:\CodexDeps\lora_training\output_lu_bramble_clerk"
$log = "D:\CodexDeps\lora_training\train_lu_bramble_clerk_proof.log"
...
try {
  ...
  & "$venv\accelerate.exe" launch `
    ...
    --train_data_dir="$data" `
    --output_dir="$out" `
    --output_name="lu_bramble_clerk_proof" `
  ...
  $ErrorActionPreference = "Stop"
  if ($exitCode -ne 0) {
    throw "Bramble proof LoRA training failed with exit code $exitCode. See $log"
  }
}
finally {
  Pop-Location
